...
def getDBPath():
  ...
  return None if dlg.DoModal() != win32con.IDOK else dlg.GetPathNames()[0]
...
class Col:
  def __init__(self, mainCol=mainCol, mainColLock=mainColLock):
    self.mainCol = mainCol
    self.mainColLock = mainColLock

  def __enter__(self) -> _Collection:
    self.mainColLock.acquire()
    return self.mainCol

  def __exit__(self, type, value, trace_back):
    self.saveMainCollection()
    self.mainColLock.release()

  @debounce(1.0)
  def saveMainCollection(self):
    self.mainColLock.acquire()
    try:
      self.mainCol.save()
    finally:
